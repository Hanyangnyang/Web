...
export const createLibraryRepository = (
  ...
  getStatus: async () => {
    ...
    if (!Array.isArray(res.data?.readingRooms))
      throw apiError(`library API returned invalid shaped 'data': ${JSON.stringify(res.data?.readingRooms)}`, { area: AREA, endpoint: res._requestUrl });

    // 3. 열람실 목록이 하나도 없을때, Error 반환
    ...
    if (res.data.readingRooms.length === 0)
