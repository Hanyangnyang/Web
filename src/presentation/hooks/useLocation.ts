...
let inflight: Promise<Coords> | null = null; // 진행 중인 측위 Promise — 중복 요청을 하나로 합침

const isFresh = (c: Coords | null): c is Coords => !!c && Date.now() - c.timestamp < MAX_AGE_MS;
...
const isTimeoutError = (error: unknown): boolean => {
  ...
  return code === NATIVE_TIMEOUT_CODE || code === WEB_TIMEOUT_CODE;
};
...
async function measureWithRetry(attempt = 1): ReturnType<typeof getCurrentPosition> {
  try {
...
function measure(): Promise<Coords> {
...
export function useLocation(active: boolean): UseLocationResult {
  const [measured, setMeasured] = useState<Coords | null>(null);
