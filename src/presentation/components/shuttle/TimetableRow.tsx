...
import { SUBWAY_OPTS, connectingTrains, isSubwayOffPeak, isSubwayScheduleMissing, type SubwayScheduleRow } from '../../../domain/entities/Subway.js';
...
export function TimetableRow({ row, lineId, isNext, isLast, isPast, subwayArrivals, isSubwayLoading, isSubwayError, hideSubwayCol, now, isFullMode, isActiveInFull, shouldScroll, autoFlip }: TimetableRowProps) {
  ...
  // 조회 실패(확인 불가) / 노선 데이터 자체가 없음(정보 없음, 시간표 개정 미반영 등 예외) / 막차 지남(운행 시간 외) /
  // 그 외 단순히 연결되는 열차가 없음을 각각 구분해서 안내 (재시도 유도는 위쪽 배너 한 곳에서만)
  const noTrainReason = row.subway && trains.length === 0
    ? (isSubwayError ? '확인 불가'
      : isSubwayScheduleMissing(subwayArrivals, lineId) ? '정보 없음'
