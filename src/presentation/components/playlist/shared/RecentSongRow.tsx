...
interface RecentSongRowProps {
  song: Song;
  // 오른쪽 화살표 클릭 — 최근 추가된 곡 전체보기로 이동해서 이 곡으로 스크롤
  onSelect: (song: Song) => void;
  // 앨범커버와 곡 정보 영역 클릭 — 바로 재생
  onPlay: (song: Song) => void;
  // 지금 하단 플레이어에서 재생 중인 곡 — 앨범커버의 재생 아이콘이 일시정지 아이콘으로 바뀜
  currentTrackId?: string | null;
}

// 최근 추가된 곡 홈 미리보기 행 — 앨범커버와 곡 정보 영역을 누르면 바로 재생되고,
// 오른쪽 화살표만 누르면 최근 추가된 곡 전체보기로 이동해서 이 카드 위치로 스크롤됨
  ...
  return (
    <div className="relative flex items-stretch bg-white rounded-card border border-slate-200 shadow-[0_2px_4px_rgba(0,0,0,0.03)] overflow-hidden">
      ...
      >
        ...
        />
        {/* 기본 재생 아이콘은 숨기고, 실제 재생 중일 때만 일시정지 아이콘을 표시한다. */}
        {isPlaying && (
