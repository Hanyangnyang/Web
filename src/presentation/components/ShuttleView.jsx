      ...
      <div className="sticky top-0 bg-[#F8F9FA]/80 backdrop-blur-xl z-[100] -mx-5 px-5 pt-4 pb-4 border-b border-[#e2e8f0]/50 shadow-[0_4px_12px_rgba(0,0,0,0.03)] mb-6">
        ...
        <div className="grid grid-cols-3 gap-2">
          {STOPS.map((s, idx) => (
            <div
              key={s}
              className={`py-[7px] px-2 text-center flex items-center justify-center gap-1 border-[1.5px] rounded-full text-[13px] font-semibold cursor-pointer whitespace-nowrap transition-all duration-150 shadow-[0_2px_4px_rgba(0,0,0,0.02)] relative ${
                stop === s
            ...
            >
              {initialStop === s && showTooltip && (() => {
                const col = idx % 3;
                const alignClass = col === 0 ? 'align-start' : col === 2 ? 'align-end' : '';
                const tx = col === 0 ? '-25%' : col === 2 ? '-75%' : '-50%';
                return (
                <div
                  className={`stt-tooltip bottom ${alignClass} absolute left-1/2 bg-[rgba(33,37,41,0.9)] text-white px-3.5 py-2.5 rounded-card text-[11px] font-bold whitespace-nowrap shadow-[0_12px_24px_-6px_rgba(0,0,0,0.3)] z-[500] flex items-center pointer-events-none backdrop-blur-sm transition-all duration-400 ${isTooltipFadingOut ? 'opacity-0' : ''} [animation:tooltipPopDown_0.4s_cubic-bezier(0.175,0.885,0.32,1.275)] top-[calc(100%+12px)] bottom-auto`}
                  style={{ transform: `translateX(${tx})${isTooltipFadingOut ? ' translateY(0.5rem)' : ''}` }}
                >
