...
import { INSTA_ACCOUNTS, type InstagramAccountInfo } from '../../../domain/entities/InstagramAccount.js';
...
function AccountItem({ acc, isFirst }: AccountItemProps) {
...
function AccountGroup({ groupKey, title, accounts, expanded, onToggle }: AccountGroupProps) {
  ...
  return (
    ...
    >
      {accounts.map((acc, idx) => (
        <AccountItem key={acc.username} acc={acc} isFirst={idx === 0} />
      ))}
    </Accordion>
  );
}
  ...
  return (
    <div className="pb-20">
      ...
      <div className="[animation:slideUp_0.4s_ease-out]">
        <AccountGroup groupKey="erica" title="에리카" accounts={INSTA_ACCOUNTS.erica} expanded={expanded} onToggle={toggle} />
        <AccountGroup groupKey="college" title="단과대학" accounts={INSTA_ACCOUNTS.college} expanded={expanded} onToggle={toggle} />
      </div>
    </div>
  );
}
