// 중앙동아리 제보하기 팝업 — 동아리 정보가 실제와 다르거나 인스타 등 정보가 비어있을 때 자유 텍스트로 제보
import { useState } from 'react';
import { useSubmitFeedbackApi } from '../../hooks/useSubmitFeedbackApi.js';

interface ClubFeedbackModalProps {
  onClose: () => void;
}

const MAX_LENGTH = 300;

export function ClubFeedbackModal({ onClose }: ClubFeedbackModalProps) {
  ...
  const handleSubmit = () => {
    ...
    submitFeedback.mutate(
      { category: 'CLUB', feedbackType: 'INACCURACY', content: content.trim() },
  ...
  return (
    <div
      className="fixed inset-0 z-[1100] flex items-center justify-center bg-black/40 px-8"
      onClick={onClose}
    >
      <div
        ...
        {submitted ? (
          ...
          <>
            ...
            <textarea
              ...
              placeholder="OO 동아리 인스타 계정명 OO인데 추가해주세요!"
              rows={4}
